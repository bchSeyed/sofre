jQuery(document).ready(function($) {
    ...
    $('#sf-toggle-restaurant').on('click', function() {
        var btn = $(this);
        btn.prop('disabled', true);
        
        $.post(sf_ajax.ajax_url, {
            action: 'sf_toggle_restaurant',
            nonce: sf_ajax.nonce
        }, function(response) {
            if (response.success) {
                var isOpen = response.data.status === 'yes';
                btn.removeClass('open closed').addClass(isOpen ? 'open' : 'closed');
                btn.find('.sf-toggle-text').text(isOpen ? 'باز' : 'بسته');
                btn.data('current', isOpen ? 'open' : 'closed');

                var banner = $('#sf-status-banner');
                if (banner.length) {
                    banner.removeClass('sf-status-open sf-status-closed')
                          .addClass(isOpen ? 'sf-status-open' : 'sf-status-closed');
                    banner.find('.sf-status-icon').html(isOpen ? '🟢' : '🔴');
                    banner.find('.sf-status-msg').text(
    ...
    function startOrderPolling() {
        ...
        var firstRow = $('.sf-order-row').first();
        if (firstRow.length) {
    ...
    $('.sf-order-status').on('change', function() {
        var select = $(this);
    ...
    $(document).on('click', '.sf-view-order', function() {
        var orderId = $(this).data('order-id');
        showOrderDetail(orderId);
    });

    function showOrderDetail(orderId) {
        var modal = $('#sf-order-modal');
        modal.show();
        $('#sf-modal-order-number').text('#' + orderId);
        $('#sf-modal-body').html(
            '<div class="sf-loading" style="text-align:center;padding:40px;">' +
            '<div class="sf-spinner"></div><p>در حال بارگذاری...</p></div>'
        );

        $.post(sf_ajax.ajax_url, {
            action: 'sf_get_order_detail',
            order_id: orderId,
            nonce: sf_ajax.nonce
        }, function(response) {
            if (response.success) {
                ...
                $('#sf-modal-body').html('<p style="color:red;text-align:center;padding:40px;">خطا در دریافت اطلاعات</p>');
            }
        });
    }
    ...
    function renderOrderDetail(data) {
        ...
        html += '<div class="sf-order-section">';
        html += '<h3>👤 اطلاعات مشتری</h3>';
        html += '<table class="sf-detail-table">';
        ...
        html += '<a href="' + sf_ajax.ajax_url.replace('admin-ajax.php', 'post.php?post=' + data.id + '&action=edit') + '" class="button button-primary" target="_blank">مشاهده در ووکامرس</a>';
        html += '</div>';

        $('#sf-modal-body').html(html);
    }

    $(document).on('click', '.sf-modal-close, .sf-modal-overlay', function() {
        $('#sf-order-modal').hide();
    });
    ...
    $('#sf-upload-logo').on('click', function(e) {
        e.preventDefault();
        ...
        bqFileFrame.on('select', function() {
            ...
            $('#sf_restaurant_logo').val(attachment.url);
            $('#sf-logo-preview').html('<img src="' + attachment.url + '" style="max-width:150px;max-height:80px;">');
            $('#sf-remove-logo').show();
        });
    ...
    $('#sf-remove-logo').on('click', function() {
        $('#sf_restaurant_logo').val('');
        $('#sf-logo-preview').html('<span class="sf-logo-placeholder">لوگو انتخاب نشده</span>');
        $(this).hide();
    });

    // ============ 7. شروع Polling ============
    
    if ($('.sf-dashboard').length || $('.sf-orders-wrap').length) {
